...
static const float TQ3_0_CENTROIDS_NEON[8] = {
    -2.1519f, -1.3439f, -0.7560f, -0.2451f,
     0.2451f,  0.7560f,  1.3439f,  2.1519f
};

static const float TQ3_0_SIGNS_NEON[32] = {
...
static inline void dequantize_block_tq3_0_neon(const block_tq3_0 * GGML_RESTRICT x, float32x4_t * v) {
    ...
    for (int i = 0; i < 8; i++) {
        v[i] = vsetq_lane_f32(TQ3_0_CENTROIDS_NEON[indices[i*4 + 0]], v[i], 0);
        v[i] = vsetq_lane_f32(TQ3_0_CENTROIDS_NEON[indices[i*4 + 1]], v[i], 1);
        v[i] = vsetq_lane_f32(TQ3_0_CENTROIDS_NEON[indices[i*4 + 2]], v[i], 2);
        v[i] = vsetq_lane_f32(TQ3_0_CENTROIDS_NEON[indices[i*4 + 3]], v[i], 3);
    }
    ...
    for (int i = 0; i < 8; i++) {
        float32x4_t signs = vld1q_f32(TQ3_0_SIGNS_NEON + i*4);
...
void dequantize_row_tq3_0(const block_tq3_0 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        float32x4_t v[8];
        dequantize_block_tq3_0_neon(&x[i], v);
        for (int j = 0; j < 8; j++) {
            vst1q_f32(y + i*QK_TQ3_0 + j*4, v[j]);
        }
    }
#else
    dequantize_row_tq3_0_ref(x, y, k);
#endif
}
...
void ggml_vec_dot_tq3_0_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; i++) {
        float32x4_t v_tq[8];
        dequantize_block_tq3_0_neon(&x[i], v_tq);

        const float d_y = GGML_CPU_FP16_TO_FP32(y[i].d);
        
        int8x16_t q8_0 = vld1q_s8(y[i].qs);
        int8x16_t q8_1 = vld1q_s8(y[i].qs + 16);

        // Block 0-7
        #define DOT_Q8_BLOCK(q8_low_high, tq_idx) { \
            int16x8_t q8_16 = vmovl_s8(q8_low_high); \
            float32x4_t q8_f_low  = vcvtq_f32_s32(vmovl_s16(vget_low_s16(q8_16))); \
            float32x4_t q8_f_high = vcvtq_f32_s32(vmovl_s16(vget_high_s16(q8_16))); \
            sumv = vmlaq_n_f32(sumv, vmulq_f32(v_tq[tq_idx],   q8_f_low),  d_y); \
            sumv = vmlaq_n_f32(sumv, vmulq_f32(v_tq[tq_idx+1], q8_f_high), d_y); \
        }

        DOT_Q8_BLOCK(vget_low_s8(q8_0),  0);
        DOT_Q8_BLOCK(vget_high_s8(q8_0), 2);
        DOT_Q8_BLOCK(vget_low_s8(q8_1),  4);
        DOT_Q8_BLOCK(vget_high_s8(q8_1), 6);
